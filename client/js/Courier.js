...
const courier = async (productId, location, timearrived) => {
    await contract.methods.courier(productId, location, timearrived).send({
        from: account
    }, (error,result) => {
        if(error){
            console.error(error)
...
const printValues = (e) => {
    ...
    courier(product_id, location, time_arrived)
  }
  
  document.getElementById("form4").addEventListener("submit", printValues);